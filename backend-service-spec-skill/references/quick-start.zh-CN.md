...
- `requirement_fact_map`：在历史需求缺失时，按功能模块抽取当前事实需求
...
4. 历史需求缺失时补 `requirement_fact_map`
5. 最后 `build_domain_map`
...
```text
requirement_fact_map: scope=<system-or-module>, goal=<输出事实需求、证据矩阵、需求缺口>
```
...
- `requirement_fact_map`：事实需求索引、模块需求页、需求证据矩阵、需求缺口页
...
- [使用指南](./usage-guide.md)
- [命令产物对照](./command-output-map.zh-CN.md)
- [事实需求抽取](./requirement-fact-extraction.md)
